        ...
        controller: function ($scope,$state,$location, $rootScope) {
           ...
           var tree = $rootScope.cattree;
                ...
                if (statename == 'category') {
                     var result = _.where(tree, {Name: label});  
                     console.log(result);               
                    $state.go('category' , {ID:result[0].ID});
                }
                if(statename == 'secound'){
                    var result = _.where(tree, {Name: label});
                    var subId = result[0].ID;
                    var parentCat =  _.where(tree, {ID: result.ParentID});
                    $state.go('secound', {ID:parentCat[0].ID,subID:result[0].ID});
                }
