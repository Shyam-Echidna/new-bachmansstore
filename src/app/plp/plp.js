...
function PlpController(BaseService, $rootScope, FacetList, FiltersObject, CurrentCatgory, ProductResultsWithVarients, Selections, ProductSearchResult, SharedData, $state, $uibModal, $q, Underscore, $stateParams, PlpService, /*productList, */$scope, alfcontenturl, OrderCloud, $sce, alfStaticContenturl, PdpService) {
    var vm = this;
    ...
    /*  $scope.$emit("CurrentCatgory3", CurrentCatgory.Name);
      ...
      $scope.$emit("CurrentCatgory2", parentCat);
      if(parentCat.ParentID !=null){
       var rootCat =  _.where(tree, {ID: parentCat.ParentID});
       $scope.$emit("CurrentCatgory1", rootCat);   
      }*/
    ...
    PlpService.GetPlpBannerAlf(ticket).then(function (res) {
        ...
        if (res.items.length > 0) {
            ...
            clpContentPlp.push({
                contentUrl: 'assets/images/placement_for_PLP_ASpot.jpg',
    ...
    function multirecipient(id) {
        var modalInstance = $uibModal.open({
            ...
            resolve: {
                ...
                Order: function ($rootScope, $q, $state, CurrentOrder, $cookieStore) {
                    var dfd = $q.defer();
                    CurrentOrder.Get()
                        .then(function (order) {
                            dfd.resolve(order)
